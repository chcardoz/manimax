...
pytestmark = requires_ffmpeg_and_ffprobe
...
def _extract_frame(mp4: Path, frame_idx: int) -> np.ndarray:
    return extract_frame_raw(mp4, frame_idx, width=WIDTH, height=HEIGHT, pix_fmt="rgb24")
...
def test_ffprobe_metadata(rendered_mp4: Path) -> None:
    meta = ffprobe_stream(
        rendered_mp4,
        "width,height,avg_frame_rate,codec_name,pix_fmt,duration,nb_read_frames",
    )
...
def _check_centroids(
    ...
    for color, (exp_x, exp_y), band in (
        ...
        res = centroid_in_band(arr, band[0], band[1], band[2])
...
def _sum_and_nonzero(arr: np.ndarray) -> tuple[int, int]:
    return int(arr.sum()), int(np.count_nonzero(arr))


def test_frame_15_pixel_checksum_and_centroids(rendered_mp4: Path) -> None:
    arr = _extract_frame(rendered_mp4, 15)
    total, nonzero = _sum_and_nonzero(arr)
    ...
    _check_centroids(arr, EXPECTED_F15_CENTROIDS, "f15")


def test_frame_45_pixel_checksum_and_centroids(rendered_mp4: Path) -> None:
    arr = _extract_frame(rendered_mp4, 45)
    total, nonzero = _sum_and_nonzero(arr)
    ...
    _check_centroids(arr, EXPECTED_F45_CENTROIDS, "f45")
...
def test_frame_84_post_remove_active_set(rendered_mp4: Path) -> None:
    ...
    blue = centroid_in_band(arr, *BLUE_BAND)
    ...
    yellow = centroid_in_band(arr, *YELLOW_BAND)
