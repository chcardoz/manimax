...
## F7. PyO3 boundary error chain + `From` impls

Tracked in `docs/performance.md` E3c (perf-adjacent because it's a
boundary cleanup that affects user-facing debug experience). Listed
here as a cross-reference so anyone scanning future directions
finds it.
